...
A Unity VR prototype for cervical rehabilitation on Meta Quest. Gameplay is driven by head and neck movement — where you look, how you tilt, how you turn. Easy mode uses a controller to move forward; Medium and Hard need nothing but your head.

The idea is simple: wrap therapeutic exercises (yaw, pitch, roll) inside a game so patients actually do them.

---

## How it works

All interaction goes through gaze. A raycast fires from the camera's forward direction and hits objects that implement `IGazeable`. To select something, you hold your gaze on it for a few seconds — a visual progress indicator fills up, then the action fires. There are no physical inputs required during gameplay.

Controllers are still supported for navigating menus, but the game itself is entirely head-driven.

---

## Scenes and game modes

**Main Menu** — Gaze-driven menu. Select a difficulty, confirm, and the scene loads.

**Easy — Forest walk**
The player walks through a forest using a controller to move forward. Apples appear on both sides — you collect them by looking at them. Obstacles come from ahead; tilt your head to dodge. The session ends when you've collected all 20 apples. Score is shown in a results screen that appears wherever you're looking when the game ends.

**Medium — Worm tunnel**
You guide a worm through a procedurally generated tunnel using gaze. The worm follows your head direction. The session runs on a timer (default 60 seconds). At the end it shows your score and gaze accuracy — how long you actually kept your gaze on target vs. total session time.

**Hard — Maze**
A flat maze is projected in front of you. A laser pointer follows your head movement. You drag the laser through the maze to find the exit. You pick the session duration before starting, and the game shuffles through multiple maze layouts. It counts how many mazes you complete and how much time you took.
...
- `MouseGazeSimulator` works in editor; it disables itself automatically on device
